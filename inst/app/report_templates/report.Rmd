...
```{r}
QCcorrection:::ui_basic_info(params$choices$raw_df, params$choices$replacement_counts)
...
```{r}
QCcorrection:::ui_postcor_filter_info(params$choices$filtered_corrected, params$choices$rsd_filter, params$choices$post_cor_filter)
```
...
### Candidate Outliers
```{r}
statement("Candidate Outliers")
```

```{r}
QCcorrection:::ui_outliers(list(out_data = params$choices$out_data, sample_grouping = params$choices$sample_grouping), list(filtered_corrected = params$choices$filtered_corrected, transformed = params$choices$transformed))
```
