test_that("clean_data basic cleaning and outputs", {
  ...
  expect_true(is.unsorted(df$Injection))           # input unsorted
  expect_equal(out$df$order, sort(out$df$order))            # output sorted
  ...
  expect_true(any(is.na(out$df$met1)))             # from "a" and 0→NA
  expect_true(any(is.na(out$df$met2)))             # from "foo" and 0→NA
  expect_true(all(is.na(out$df$met3)))             # all zeros→NA
...
test_that("clean_data errors when first sample after sort is not QC", {
  ...
  expect_error(
    clean_data(df, "SampleID","BatchID","Type","Injection", withheld_cols = character()),
...
test_that("withheld cols are not present in df but are tracked", {
  ...
  out <- clean_data(df, "SampleID","BatchID","Type","Injection", withheld_cols = "keep_me")
