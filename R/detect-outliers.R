...
detect_qc_aware_outliers <- function(df,
  ...
  chisq_cut <- function(k, q = md_cutoff_quantile)
    stats::qchisq(q, df = k)
  ...
  flagged_samples <- subset(md_tbl, flagged &
                              group_id != "QC")$sample
  ...
  # Confirmation per group - metabolite
  confirm_rows <- list()
  if (nrow(cand_long) > 0) {
    ...
    for (idx in split_idx) {
      ...
      for (r in seq_len(nrow(rows))) {
        ...
        if (confirm_method == "dixon") {
          ...
          cand_val <- vals[s]
          at_extreme <- (which.min(x) == which(names(x) == s)) ||
            (which.max(x) == which(names(x) == s))
          if (at_extreme) {
            tt <- tryCatch(
