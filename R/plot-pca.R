...
plot_pca <- function(p, before, after, compared_to) {
  ...
  # Custom color palette (for discrete scales)
  cbPalette <- c(
    "#F3C300",
    "#875692",
    "#ee7733",
  ...
  # Build color scale function depending on whether we want gradient or discrete palette
  if (use_gradient) {
    ...
    scale_color_pca <- function() {
      ggplot2::scale_color_viridis_c(
        option = "viridis",   # or "plasma", "magma", "inferno", "cividis"
        limits = order_range,
        name   = col
      )
    }
  } else {
    # Discrete palette handling (existing behavior)
