...
plot_class_rsd_distributions <- function(df_before,
  ...
  rsd_before <- class_metabolite_rsd(df_before)
  rsd_after <- class_metabolite_rsd(df_after)
  # Basic checks
  required_cols <- c("class", "Metabolite", "RSD")
  if (!all(required_cols %in% names(rsd_before))) {
    stop("`rsd_before` must contain columns: ", paste(required_cols, collapse = ", "))
  }
  if (!all(required_cols %in% names(rsd_after))) {
  ...
  rsd_all$dataset <- factor(rsd_all$dataset,
                            levels = c(before_label, after_label))
  
  # Long format: one column for QC vs NonQC, one for RSD values
  rsd_all$type <- ifelse(rsd_all$class == "QC", "QC", "Samples")
  
  # Factor so Samples is left panel, QC right panel
  rsd_all$type <- factor(
    rsd_all$type,
    levels = c("Samples", "QC")
  )
  ...
  p <- ggplot2::ggplot(rsd_all, ggplot2::aes(x = RSD, fill = dataset, color = dataset)) +
    ggplot2::facet_wrap(~ type, nrow = 1, scales = "fixed") +
    ggplot2::geom_density(
