...
export_figures <- function(p, d, out_dir = tempdir()) {
  ...
  mk <- function(subdir) { x <- file.path(fig_dir, subdir); if (!dir.exists(x)) dir.create(x, recursive = TRUE); x }
  ...
  save_plot <- function(path, plot, w, h) {
    if (fmt == "png") {
      ggplot2::ggsave(path, plot = plot, width = w, height = h, units = "in", dpi = 300, bg = "white")
    } else {
      ggplot2::ggsave(path, plot = plot, width = w, height = h, units = "in", device = grDevices::cairo_pdf)
    }
    normalizePath(path, winslash = "/", mustWork = TRUE)
  }
  
  raw_cols <- setdiff(names(d$filtered$df), c("sample","batch","class","order"))
  cor_cols <- setdiff(names(d$filtered_corrected$df), c("sample","batch","class","order"))
  ...
  shiny::withProgress(message = "Creating figures...", value = 0, {
    ...
    pca_path <- file.path(pca_dir, sprintf("pca_comparison_%s.%s", p$color_col, fmt))
