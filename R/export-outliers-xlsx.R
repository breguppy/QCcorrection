...
export_outliers_xlsx <- function(p, d, file = NULL) {
  ...
  shiny::withProgress(message = "Creating candidate_outliers_*today's_date*.xlsx...", value = 0, {
    s1 <- .add_sheet("Sample MD")
    txt1 <- paste(
      "Tab 1. This tab shows sample name, group ID, squared Mahalanois distances, Chi-squared quantile cutoff, and True/False outlier flag.",
