...
export_outliers_xlsx <- function(p, d, file = NULL) {
  ...
  txt1 <- paste("Tab 1. This tab shows sample name, group ID, squared Mahalanois distances, Chi-squared quantile cutoff, and True/False outlier flag.",
  ...
  txt2 <- paste("Tab 2. This tab shows sample name, batch, class, order, group ID, metabolite, and robust z-score for all candidate metabolite outliers.",
  ...
  txt3 <- paste("Tab 3. This tab shows sample name, batch, class, order, groupID, metabolite, robust z-score, QC RSD, outlier test method, p value, and decision on all candidate outliers from the previous tab.",
